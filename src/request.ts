import { buildRequestBody, parseJSON, resetProps } from './helpers'
...
class Credentials {
  ...
  constructor(client_id: string, access_token: string, expires: number) {
...
class RequestFactory {
  ...
  authenticate() {
    ...
    const promise = new Promise<AuthResponse>((resolve, reject) => {
      config.auth.fetch
        .bind(undefined)(`${config.protocol}://${config.host}/${config.auth.uri}`, {
          method: 'POST',
          headers: {
  ...
  send(uri: string, method: string, body: any = undefined, token: string | undefined = undefined, instance: any) {
    const { config, storage } = this

    const promise = new Promise((resolve, reject) => {
      const credentials = JSON.parse(storage.get('moltinCredentials'))
      const req = ({ access_token }: { access_token: string }) => {
